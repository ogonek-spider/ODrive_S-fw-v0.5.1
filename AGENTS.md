...
### Relaxed `ABS_SPI_COM_FAIL` threshold

`Firmware/MotorControl/encoder.cpp` raises the SPI miss-rate that trips
`ERROR_ABS_SPI_COM_FAIL` for the `MODE_SPI_ABS_*` encoders from `0.005f` (0.5%)
to `0.05f` (5%). An MT6701 on flying leads picks up motor PWM EMI: ~0.5% missed
SPI samples at steady spin and up to ~24% during the motor-cal inductance (HF
current) injection at 5 A / 4 V, which tripped the original threshold and made
motor/encoder calibration impossible. A truly disconnected encoder is ~100%
misses, so 5% still detects it. Mitigations that also help: route encoder
signal away from phase wires + ferrite on the power leads, and calibrate with a
gentle current (`calibration_current=3.0`, `resistance_calib_max_voltage=2.0`,
